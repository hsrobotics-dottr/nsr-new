...
keywords:
'HSS-LDE-F4, F4 Series, Servo Sürücü, Yüksek Performans, Endüstriyel Otomasyon',
title: 'F4 Serisi Sürücü',
description: `F4 serisi servo sürücü, endüstriyel otomasyon için yüksek performanslı, yüksek yanıtlı, kolay kullanılabilir bir servo ürünüdür. Gelişmiş düşük frekanslı titreşim bastırma algoritması, akım döngüsü parametre otomatik ayarlama, sürtünme öz-öğrenme, rijitlik seviyesi ayarlama ve diğer algoritmaları birleştirir. Basit ve verimli çalışır ve çoğu yüksek hassasiyetli metal işleme için doğruluk gereksinimlerini karşılayabilir. Bağımsız ısı dağıtım hava kanalı tasarımı kullanarak toz, yüksek nem ve diğer sızıntıların sürücü gövdesine girmesini önler ve zorlu endüstriyel ortamlarla kolayca başa çıkabilir.`,
image: '/img/ServoDrives/HSS-LDE-F4.png',
    specifications: {
    model: 'HSS-LDE-F4',
type: 'F4 Serisi Servo Sürücü',
powerRange: '4.5A - 9.7A',
maxCurrent: '24.5A',
axisCount: '4 Eksen',
installation: 'Dikey Kurulum',
},
  driveModels: {
'HSS-LDE-F4-045': { ratedCurrent: '4.5A', maxCurrent: '13.5A' },
'HSS-LDE-F4-082': { ratedCurrent: '8.2A', maxCurrent: '21A' },
'HSS-LDE-F4-097': { ratedCurrent: '9.7A', maxCurrent: '24.5A' },
},
    inputSpecs: {
    ratedVoltage: 'Tek faz 220V (güç azaltma)/üç faz 220V',
controlVoltage: '24V',
},
mainFeatures: [
'4 eksen entegrasyonu, dikey kurulum, yüksek entegrasyon, küçük boyut, daha az kablo bağlantısı',
'DC veri yolu çıkış arayüzü sağlar, kolay genişletme',
'Düşük frekans ve yüksek frekans titreşim bastırma algoritması 50Hz-5KHz titreşimi etkili şekilde bastırabilir',
'Akım döngüsü otomatik ayarlama teknolojisi, hızlı motor adaptasyonu',
'Çeşitli kodlayıcı protokollerine uyum sağlayabilir, özel geliştirmeyi destekler',
'EtherCAT veri yolu karıştırma ve eşleştirme',
'Rijitlik seviyesi ayarlama, basit ve verimli çalışma, müşterilerin ayarlama yapması daha kolay',
'4 eksen parametreleri birbirini kopyalar, hızlı parametre ayarlama sağlar ve çok eksenli ayarlama süresini büyük ölçüde tasarruf eder',
'Farklı yük eylemsizlik momenti tanımlamasına göre, daha hızlı ve doğru performans ayarlama',
'Zayıf manyetik kontrol teknolojisi, hızı büyük ölçüde artırabilir, motorun nominal maksimum hızına kadar',
'Yerçekimi adaptif kompanzasyon yeteneği, üst etkinleştirme anlık dış kuvvetlerden etkilendiğinde meydana gelen "baş sallama" olayını engelleyebilir',
'Bağımsız hava kanalı tasarımı, elektronik cihazların etkili korunması',
],
    communication: {
    standard: ['EtherCAT'],
...
undervoltage: 'Evet',
dustProtection: 'Evet',
humidityProtection: 'Evet',
},
notes: [
'Yukarıdaki tablodaki akım AC akım ise, aksi belirtilmedikçe etkin değer olarak kabul edilir.',
'Model seçimi motor nominal akımı ve sürücü çıkış nominal akımı referans seçimine göre yapılır.',
'Tek faz 220V güç kaynağı hattı seçilmesi gerekir ve 2.5 mm² kablo gereklidir.',
],
};
...
dustProtection: 'Toz Koruması',
humidityProtection: 'Nem Koruması',
notes: 'Notlar',
...
humidityProtection: 'Humidity Protection',
notes: 'Notes',
...
export default function HSSLDEF4Page() {
...
{/* Product Image */}
      <div
        className='relative'>
      <div
        className='bg-gray-50 rounded-2xl p-4 flex items-center justify-center min-h-[200px]'>
<Image
...
{productData.inputSpecs.ratedVoltage}
      </span>
      </div>
      <div
        className='flex justify-between py-2 border-b border-gray-100'>
...
{t.product.controlVoltage}
      ...
      <span
        className='font-semibold text-gray-800 text-sm'>
{productData.inputSpecs.controlVoltage}
      </span>
      </div>
      </div>
      </Card>
...
<div key={index} className='flex items-start'>
      <div
        className='w-2 h-2 bg-yellow-500 rounded-full mt-2 mr-3 flex-shrink-0'>      </div>
      <p
        className='text-gray-700 text-sm leading-relaxed'>
{note}
      </p>
