...
// CR Serisi robot modelleri;
const crSeriesRobots = [
{
model: 'HSR-CR605-790',
payload: '5kg',
reach: '790mm',
...
features: ['İşbirlikçi Güvenlik', 'Kolay Programlama', 'Kompakt Tasarım'],
...
'Otomotiv Endüstrisi - Montaj ve Test İşlemleri',
'Elektronik Sektörü - Hassas Montaj ve Kalite Kontrol',
...
'Eğitim ve Araştırma - Robotik Öğretim',
'Küçük ve Orta Ölçekli İşletmeler - Esnek Üretim',
'İnsan-Robot İşbirliği - Güvenli Çalışma Ortamları',
];

export default function CRSeriesPage() {
...
Ürün Merkezi
      ...
      <h1
        className='text-4xl md:text-6xl font-bold mb-6'>CR Serisi      </h1>
      <p
...
İnsan-robot işbirliği için özel olarak tasarlanmış CR serisi;
robotlar, güvenlik ve esneklik sunan çözümler sağlar.
      </p>
      </div>
      </div>
      </section>
...
CR serisi işbirlikçi robotlar, güvenli insan-robot işbirliği;
gerektiren alanlarda yaygın olarak kullanılmaktadır.
      </p>
      </div>
...
uygun işbirlikçi robot seçimi yapın.
      </p>
      </div>

      <div
        className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8'>
{crSeriesRobots.map((robot, index) => (
<Card
key={index}
...
{robot.reach}
      ...
      <div
        className='pt-4'>
<Link
href={`/products/cr-series/${robot.model
.toLowerCase()
.replace(/\//g, '-')
.replace(/-/g, '-')}`}
...
CR Serisi Hakkında Daha Fazla Bilgi
      </h2>
      <p
...
CR Serisi hakkında bilgi almak için formu doldurun.
      </p>
      </div>
...
rows={4}
className='w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent'
placeholder='CR Serisi hakkında bilgi almak istiyorum...'
>      </textarea>
      </div>
